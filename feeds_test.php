...
<main id="main" class="main">
    <div class="pagetitle d-flex">
        ...
        <div style="margin-left:auto;">
            <a href="post.php"><button class="btn btn-primary" data-target="#new-task-modal" data-toggle="modal" style="height:40px;"> Make Post</button></a>
        </div>
    </div>
    <!-- End Page Title -->
...
<main id="main" class="main">
   <div class="pagetitle d-flex">
      ...
      <!-- <button class="btn btn-primary" data-target="#new-task-modal" data-toggle="modal" style="margin-left: auto;height:50px;"><a href="post.php">Make Post</a></button> -->
   </div>
   <!-- End Page Title -->
   <section class="section dashboard">
      <div class="row justify-content-center">
                  ...
                  <div class="d-flex justify-content-between">
                     <div class="user-info">
                        <h5 class="card-title"><?php 
                           echo htmlspecialchars($post['username']); ?></h5>
                     </div>
                     <!-- <div >
                        <button class="btn btn-outline-primary like-btn" data-post-id="<?php 
                           // echo $post['post_id']; ?>">
                  ...
                  <div class="post-content">
                     ...
                     <div class="d-flex justify-content-between">
                        ...
                        <div >
                            <?php 
                            $stmtCheckFollow = $conn->prepare("SELECT * FROM likes WHERE user_id = ? AND post_id = ?");
                            $stmtCheckFollow->bind_param("ii", $loggedInUserId, $post['post_id']);
                            $stmtCheckFollow->execute();
                            $isFollowing = $stmtCheckFollow->get_result()->num_rows > 0;
                            $stmtCheckFollow->close();
                            ?>
                           <br>
                           <button id ="lk-btn" class="btn btn-outline-primary like-btn" data-post-id="<?php 
                              echo $post['post_id']; ?>"><?php echo $isFollowing ? 'Unlike ' : 'Like '; ?><span class="like-count"><?php 
                              echo $post['like_count']; ?></span>
                           </button>
                        </div>
                     </div>
...
//                         this.textContent = 'Unlike ' + this.textContent.split(' ')[1];
//                     } else {
//                         this.textContent = 'Like ' + this.textContent.split(' ')[1];
//                     }
                   }
               })
               .catch(error => console.error('Error:', error));
